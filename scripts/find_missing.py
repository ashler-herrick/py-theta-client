...
def format_summary_report(missing_data):
    ...
    ticker_counts = [
        (
            ticker,
            sum(count for year_data in years.values() for count in year_data.values()),
        )
...
def format_by_ticker_report(missing_data):
    ...
    for ticker in sorted(missing_data.keys()):
        ...
        total_days = sum(
            count for year_data in years_data.values() for count in year_data.values()
        )
...
def main():
    ...
    parser.add_argument("--out", help="Write text report to file instead of stdout")
    parser.add_argument(
        "--csv", help="Write CSV file with symbol, year, month, url columns"
    )
